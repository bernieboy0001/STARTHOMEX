    ...
    <main className="main app-main">
      ...
      <section className="grid-2">
        ...
        <article className="panel"><div className="panel-head"><h3>Add appointment</h3></div><form className="form" action={createVisit}>
          <input type="hidden" name="careRecipientId" value={careRecipientId} /><input name="title" placeholder="Appointment title" required /><input name="startsAt" type="datetime-local" /><input name="location" placeholder="Location" /><input name="providerName" placeholder="Provider or contact" /><textarea name="preparationNotes" placeholder="Preparation notes" />{demo ? <Link className="button" href="/sign-in">Sign in to save</Link> : <button className="button" type="submit">Add appointment</button>}
        </form></article>
      </section>
    </main>
  );
