...
export default async function FamilyPage({ searchParams }: { searchParams?: Promise<{ invite?: string }> }) {
  ...
  const { recipient, contacts, memberships, invites, demo } = await loadDashboard();
        ...
        <article className="panel"><div className="panel-head"><h3>Members and invites</h3></div><div className="rows">
          {memberships.map(member => (
            <div className="row split-row" key={member.id}>
              ...
              <form action={revokeMemberAccess}><input type="hidden" name="membershipId" value={member.id} /><input type="hidden" name="careRecipientId" value={careRecipientId} /><button className="ghost danger" type="submit" disabled={demo}>Remove</button></form>
            </div>
          ))}
          {invites.map(invite => (
            <div className="row split-row" key={invite.token}>
              ...
              {!invite.revoked_at && !invite.accepted_at && <form action={revokeInviteLink}><input type="hidden" name="token" value={invite.token} /><input type="hidden" name="careRecipientId" value={careRecipientId} /><button className="ghost danger" type="submit" disabled={demo}>Revoke</button></form>}
            </div>
          ))}
        </div></article>
      </section>
