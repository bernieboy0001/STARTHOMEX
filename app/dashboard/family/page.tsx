    ...
    <main className="main app-main">
      ...
      <section className="grid-2">
        <article className="panel"><div className="panel-head"><h3>Create invite link</h3></div>
          {canInvite ? <form className="form" action={createInviteLink}><input type="hidden" name="careRecipientId" value={careRecipientId} /><label>Invitee email <input name="invitedEmail" type="email" placeholder="Optional for a shareable link" /></label><label>Access role <select name="role" defaultValue="family_member"><option value="family_member">Family member</option><option value="home_aide">Home aide</option><option value="agency_coordinator">Agency coordinator</option><option value="clinician">Clinician</option></select></label><button className="button" type="submit">Generate invite link</button></form> : <p className="muted">Only the family lead can invite people into this care circle.</p>}
